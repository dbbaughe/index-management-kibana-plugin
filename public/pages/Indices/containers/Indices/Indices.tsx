...
export default class Indices extends Component<IndicesProps, IndicesState> {
  ...
  getIndices = async (): Promise<void> => {
    ...
    try {
      ...
      const { indices, totalIndices, error } = await indexService.getIndices(queryParamsString);
      if (!error) {
          ...
          <ModalConsumer>
            {({ onShow }) => (
              <IndexActions
                ...
                onClickAdd={() => onShow(AddPolicyModal, { indices: selectedItems.map((item: CatIndex) => item.index) })}
              />
            )}
          </ModalConsumer>
        }
