...
describe("<Indices /> spec", () => {
  it("renders the component", async () => {
    browserServicesMock.indexService.getIndices = jest.fn().mockResolvedValue({ indices: [], totalIndices: 0, error: undefined });
  ...
  it("loads indices", async () => {
    ...
    browserServicesMock.indexService.getIndices = jest.fn().mockResolvedValue({ indices, totalIndices: 1, error: undefined });
    const { getByText } = renderWithRouter(Indices);
  ...
  it("adds error toaster when get indices has error", async () => {
    browserServicesMock.indexService.getIndices = jest.fn().mockResolvedValue({ indices: [], totalIndices: 0, error: "some error" });
    renderWithRouter(Indices);
  ...
  it("can add a policy to an index", async () => {
    ...
    browserServicesMock.indexService.getIndices = jest.fn().mockResolvedValue({ indices, totalIndices: 1, error: undefined });
    browserServicesMock.indexService.searchPolicies = jest.fn().mockResolvedValue({ policies: ["some_policy"], error: undefined });
    ...
    userEvent.click(getByTestId("indexActionsAddPolicy"));
  ...
  it("sorts/paginates the table", async () => {
    ...
    browserServicesMock.indexService.getIndices = jest
      .fn()
      .mockResolvedValueOnce({ indices: indices.slice(0, 20), totalIndices: 40, error: undefined })
      .mockResolvedValueOnce({ indices: indices.slice(20), totalIndices: 40, error: undefined })
